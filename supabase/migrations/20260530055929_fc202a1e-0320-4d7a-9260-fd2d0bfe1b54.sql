...
CREATE TABLE public.ca_firm_billing_settings (
  ...
  pan text, gstin text, firm_state_code text,
  bank_name text, bank_account text, bank_ifsc text, account_holder text, upi_id text,
...
CREATE POLICY ca_billing_settings_write ON public.ca_firm_billing_settings FOR ALL TO authenticated
  USING (public.is_ca_owner(auth.uid(), ca_firm_id)) WITH CHECK (public.is_ca_owner(auth.uid(), ca_firm_id));
...
CREATE POLICY ca_services_write ON public.ca_services FOR ALL TO authenticated
  USING (public.is_ca_firm_member(auth.uid(), ca_firm_id)) WITH CHECK (public.is_ca_firm_member(auth.uid(), ca_firm_id));
...
CREATE TABLE public.ca_invoices (
  ...
  payment_terms text, notes text, upi_link text,
